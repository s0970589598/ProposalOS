...
| 順序 | 工具 | 目的 |
|---|---|---|
| **1st** | api-nexus **search_apis** (`q=<keyword>`) / OpenAPI / Postman collection — **多 keyword 廣搜**（method name / controller name / 子主題、不要只 1 個 namespace）| **不知 module_id 時必先 search**、authoritative for 文件層 / 公開 spec |
...
| **3rd-fallback** | `git fetch origin && git grep <keyword> origin/<branch>` 驗 remote | local grep 0 時必驗 remote、不要跳結論「在別 repo / 沒實作」 |
...
7. ✅ **回頭重讀自己早段寫過的 hedged claim**（multi-step session 結束前、看早期寫的「⚠️ 可能 / 推測 / 不一定」這類 hedged claim、後續 evidence 是否已 verify 反證該 claim、但忘了回頭 fix）— per Amafans EAQS 2026-05-29 翻車 #8
8. ✅ **Hedge「需 PROD / 需外部資料」前先廣搜本機 ground truth folder**（不要 default 假設「無 PROD access = 必 hedge」、本機可能已有同事 PROD pull 過的 cached verified data、grep 一下就有）— per Amafans EAQS 2026-05-29 翻車 #9
10. ✅ **Avoid pendulum over-correction**（user catch 第 1 次 correction 後、不要 reverse 走另一極端、找 middle ground、保留 user 第 1 次 correction 對的部分、不要全砍走相反方向）— per Amafans EAQS 2026-05-29 翻車 #14
...
# Checkpoint #7: 找早期寫的 hedged claim、看是否已被 verified 反證
grep -n "⚠️\|可能\|推測\|不一定\|stale" <file>   # 找 hedged claim
# → 每個 hedged claim 回頭驗：後續 session 有沒 verify 反證？有 → 該 fix

# Checkpoint #8: hedge「需 PROD / 需外部資料」前先廣搜本機 ground truth
grep -r "<keyword>" <ground-truth-folder>/      # e.g. proposal-intelligence/clients/
# → 找到別人 PROD pull verified data？→ 不需 hedge、直接引用 + cite source
# → 沒找到？→ honest hedge OK
```
...
| 6 | local grep 0 result → claim「不在 war_room、可能在 Lambda repo」、其實 controller 在 `origin/dev` remote branch、local 沒 sync | local-only grep、沒驗 remote | `git fetch origin && git grep <keyword> origin/<branch>` 驗 remote | sequence 加 3rd-fallback「remote grep」 |
| 7 | `search_apis(q=energyMonthlyReport)` 6 支 return、漏 `markHandled`（api_key 是 `anomalyEvent.markHandled` 不是 `energyMonthlyReport.markHandled`）、claim「沒 register / internal use」、其實 api_id=620 manual_verified | 一個 keyword search、沒廣搜相關 namespace | 多 keyword 廣搜（method name / controller name / 子主題 keyword 都 try）`search_apis(q=markHandled)` / `q=anomaly` / `q=<controller-method>` | sequence 加 1st 規則「search_apis 用多 keyword 廣搜、不要只 query 1 個 namespace」 |
| 8 | §4.0 寫「API Nexus dotted namespace 不一定 1-to-1 對應 PHP route、可能含 generic Tag CRUD 加總」— stale claim、是翻車 #4 時寫的、後續 search_apis verified 證明 A↔B 永遠 1-to-1、但沒回頭 fix §4.0、user 讀到「什麼意思」才發現 | session 早期寫的 hedged claim、後續 verify 反證但沒回頭 fix | session 結束前 grep file 的 `⚠️\|可能\|推測\|不一定` keyword、每個 hedged claim 回頭驗 | commit checkpoint #7「回頭重讀早段 hedged claim」|
...
| 12 | 「模組 01-02 對聯名 RFP N/A」過度簡化成「不需做」、忽略 Intelligence 維度 | framework 抽象結論預設 1D、漏 sub-dimension | 抽象結論預設拆 2D（外向 / 內向、過去 / 現在、規格 / 實作、Marketing / Intelligence）| AP-NEW-WRITE-5.d「module 01-02 拆 2 重任務」|
| 13 | 「聯名」+「RFP」揉成一個 case type | 兩個 orthogonal dimension 揉成一個 term | 拆「文件類型」(RFP / Pitch / ADR) × 「商業關係」(客戶提案 / 對等合作 / 聯名 / 內部) | AP-NEW-WRITE-5.e「聯名 ≠ RFP」(初版) |
| 14 | reverse over-correction — 把「聯名 RFP」全砍改「對等合作 RFP」、誤導本案非聯名（actually 是） | correction pendulum over-swing 到另一極端 | 加 commit checkpoint #10「avoid pendulum over-correction」、correction 後尋找 middle ground、不要 reverse 走另一極端 | AP-NEW-WRITE-5.e 升級加 2D matrix + AP-4 翻車 #14 |
| 15 | 「客戶提案 RFP」「聯名 RFP」當 case type、忽略 RFP procurement 嚴格 definition (RFP = 客戶出、供應商寫 Proposal Response) | framework 用 RFP 當 generic doc type、沒區分 RFP / Proposal / Spec / Pitch / ADR | doc type 分 6 種、嚴格區分 owner (RFP = 甲方 / Proposal = 乙方 / Spec = 雙方 / Pitch = 我方主動 / ADR / SOW) | AP-4 翻車 #15 + AP-NEW-WRITE-5.e 加 RFP procurement def warning |
| 16 | framework 過度 emphasize US-centric「RFP」、忽略台灣 B2B 軟體 / 顧問接案 慣用 5-stage 流程 | framework default 大企業 procurement、不對應台灣中小企業 SI 接案實情 | 加台灣 B2B 軟體 / 顧問接案 5-stage 流程：初步提案 → 需求訪談 → **完整提案 (SA + 功能 + 報價)** → 簽約 → 施工/驗收/上線；RFP 作 sub-stage doc (僅大企業 / 政府用) | AP-4 翻車 #16 + AP-NEW-WRITE-5.e 加台灣 B2B 軟體 / 顧問接案 流程 awareness |
