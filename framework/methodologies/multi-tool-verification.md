...
→ **預防 = multi-tool sequence + hedging default + Numbers cite + commit checkpoint (6 條) + local-fallback-to-remote**。Single-tool answer + confident phrasing + multi-step edit 沒 final cleanup + 沒 fetch remote = 高 risk anti-hallucination + redundancy + 推測「在別處」。
