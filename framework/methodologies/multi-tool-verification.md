...
→ **預防 = multi-tool sequence + hedging default + Numbers cite + commit checkpoint (8 條) + local-fallback-to-remote + 多 keyword 廣搜 + 回頭驗 hedged claim + 廣搜本機 ground truth folder**。Single-tool answer + confident phrasing + multi-step edit 沒 final cleanup + 沒 fetch remote + 一個 keyword search 沒廣搜 + 早期 hedged claim 沒回頭驗 + hedge 前沒廣搜本機 ground truth = 高 risk anti-hallucination + redundancy + 推測「在別處」+ 漏 register entry + 自己 file 內互相打架 + 過度保守 hedge。
