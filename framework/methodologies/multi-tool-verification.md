...
→ **預防 = multi-tool sequence + hedging default + Numbers cite + commit checkpoint (6 條) + local-fallback-to-remote + 多 keyword 廣搜**。Single-tool answer + confident phrasing + multi-step edit 沒 final cleanup + 沒 fetch remote + 一個 keyword search 沒廣搜 = 高 risk anti-hallucination + redundancy + 推測「在別處」+ 漏 register entry。
