...
#### Phase 1.0：**強制 WebSearch 30 min**（per AP-NEW-WRITE-1）

⚠️ **寫 proposal 前必先跑、不可跳過、含小案**。

**動作（WebSearch）每案必做、輸出依規模**：

| 案件規模 | WebSearch 動作 | Capture Plan 輸出 |
|---|---|---|
| 小案 < NT$ 50 萬 | ✅ 必做 30 min | 🟡 1 頁筆記 / 5 facts |
| 中型案 NT$ 100 萬+ | ✅ 必做（含交叉驗證）| ✅ 完整 Capture Plan 檔 |
| 大案 / 政府案 | ✅ 必做（深度 1-2 hr）| ✅ 完整 + Win Themes + 接觸計畫 |

→ 不查的風險（用了重複工具 / 整合做錯 / 講話沒重點）**不分案件大小**。
