...
引導使用者用 **13 大模組 + 8 強制檢查項** 完成一份顧問式系統提案。
...
| 類型 | 特徵 | 適用框架子集 |
|---|---|---|
| **客戶提案** | 我方賣 / 客戶買、有預算、有 RFP / RFI | 完整 13 模組 + 8 強制檢查項 |
| **投資 / Partnership pitch** | 申請投資 / startup credit、對方是投資方 | 13 模組部分 + Capture Plan 強 + Win Themes 強 |
...
| **內部立案** | 公司內部新事業 / 預算申請 | 13 模組部分 + 商務 + 風險重 |
...
1. 帶使用者讀 [模組 01 產業](../../framework/modules/01-industry-market.md)（用 Phase 1.0 蒐集的資料）
2. 帶使用者讀 [模組 02 競品](../../framework/modules/02-competitors-alternatives.md)（**必寫**、不可「待補」、per AP-NEW-WRITE-2）
3. 用 [interview-script.md](../../playbook/interview-script.md) 準備訪談（補 Phase 1.0 查不到的）
4. 訪談紀錄整理 → 對應 [模組 03 痛點](../../framework/modules/03-problem-business-goals.md)
...
6. As-Is / To-Be / 服務藍圖（[模組 04](../../framework/modules/04-as-is-to-be-service-blueprint.md)）
7. 角色與 RACI（[模組 05](../../framework/modules/05-stakeholders-raci.md)）
8. 資料盤點與整合（[模組 06](../../framework/modules/06-data-integration-governance.md)）

### Phase 3：設計期（合規 / 架構 / 方案）

9. 合規 + 資安 + AI 風險（[模組 07](../../framework/modules/07-compliance-security-ai-risk.md)）
10. 系統架構 + NFR（[模組 08](../../framework/modules/08-architecture-functional-nfr.md)）
11. 三方案 + 分階段（[模組 09](../../framework/modules/09-solution-options-phased-rollout.md)）
12. 依產業挑 [industry-addons/](../../framework/industry-addons/)

### Phase 4：商務期（報價 / 驗收 / 風險）

13. 報價 + 商務條款（[模組 10](../../framework/modules/10-procurement-pricing.md)）
14. 驗收 + 效益追蹤 + 維運（[模組 11](../../framework/modules/11-acceptance-benefits-operations.md)）
15. 風險 + 假設 + 責任邊界（[模組 12](../../framework/modules/12-risk-assumptions-red-team.md)）
