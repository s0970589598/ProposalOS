...
ProposalOS 用 **13 大模組 + 8 強制檢查項** 來防漏，並把競品前移到需求探索階段。
...
**業務功能盤點口訣**：**產銷人發財**（生產 / 銷售 / 人資 / 研發 / 財務）— 跨產業通用，訪談覆蓋自我檢查，避免只挖到一個 function 的痛。詳見 [模組 03](framework/modules/03-problem-business-goals.md)
...
| 13 | 系統設計 + MVP 交付包 | 規格 → AI 接手可開發、Phase 7、高 spec 案件用 |

→ 詳見 [framework/modules/](framework/modules/)
...
### 主檔命名規則（混合架構：proposal.md 每案必有 + deliverable per 類型）

每案**必有 `proposal.md`**（我方視角 13 模組整理）、再依案件類型加對外 deliverable：

| 案件類型 | 內部主檔 | 對外 deliverable |
|---|---|---|
| 客戶提案 | `proposal.md` | 同 `proposal.md` |
| 投資 pitch | `proposal.md`（spine）| `docs/pitch_v{N}_final.md` |
| 聯名 / 合作 RFP | `proposal.md`（spine、13 模組 audit view）| `source-documents/RFP_V{N}.md` + `rfp-audit.md` / `tracker` |
| 顧問報告 | `proposal.md`（spine）| `report.md` |
| 政府標案 | `proposal.md` | `compliance-matrix.md` 加掛 |
| 內部立案 | `proposal.md`（spine）| `business-case.md` |

⚠️ **proposal.md 不一定是對外 deliverable**、是**我方思考整理**（套 13 模組 + 8 強制檢查）。當案件已有大量 source doc（RFP / pitch / audit）時、可用 **spine/index 模式**（cross-ref 既有檔、不重寫）。

→ 完整規則 + spine 模式範例：[docs/output-locations.md](docs/output-locations.md)
...
Step 2: 用 ProposalOS framework 套 13 模組（per case repo 寫 proposal.md）
